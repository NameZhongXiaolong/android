...
/**
 * 可以设置圆角的覆盖图(遮罩效果)
 * app:radius="覆盖角度"(如果要设置为圆形就写宽度的1/2)
 * app:cover_color="边角的颜色"
 * 该控件实际上就是覆盖了一层View, 可以设置View的边角颜色
 *
...
public class RadiusCoverView extends View {
    ...
    public RadiusCoverView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RadiusCoverView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.RadiusCoverView);

        leftTopRadians = typedArray.getDimensionPixelSize(R.styleable.RadiusCoverView_radius, leftTopRadians);
        leftBottomRadians = typedArray.getDimensionPixelSize(R.styleable.RadiusCoverView_radius, leftBottomRadians);
        rightTopRadians = typedArray.getDimensionPixelSize(R.styleable.RadiusCoverView_radius, rightTopRadians);
        rightBottomRadians = typedArray.getDimensionPixelSize(R.styleable.RadiusCoverView_radius, rightBottomRadians);

        leftTopRadians = typedArray.getDimensionPixelSize(
                R.styleable.RadiusCoverView_leftTopRadius, leftTopRadians);
        leftBottomRadians = typedArray.getDimensionPixelSize(
                R.styleable.RadiusCoverView_leftBottomRadius, leftBottomRadians);
        rightTopRadians = typedArray.getDimensionPixelSize(
                R.styleable.RadiusCoverView_rightTopRadius, rightTopRadians);
        rightBottomRadians = typedArray.getDimensionPixelSize(
                R.styleable.RadiusCoverView_rightBottomRadius, rightBottomRadians);

        coverColor = typedArray.getColor(R.styleable.RadiusCoverView_coverColor, coverColor);
    }
    ...
    private Bitmap drawSector(int w, int h) {
        ...
        c.drawArc(new RectF(0, getHeight() - leftBottomRadians * 2, leftBottomRadians * 2, getHeight()), 90, 90,
                true, p);
        c.drawArc(new RectF(getWidth() - rightTopRadians * 2, 0, getWidth(), rightTopRadians * 2), 270, 90, true, p);
        c.drawArc(new RectF(getWidth() - rightBottomRadians * 2, getHeight() - rightBottomRadians * 2, getWidth(),
                getHeight()), 0, 90, true, p);
        return bm;
    }
    ...
    private Bitmap drawRect(int w, int h) {
        ...
        c.drawRect(new RectF(getWidth() - rightBottomRadians, getHeight() - rightBottomRadians, getWidth(), getHeight
                ()), p);
        return bm;
    }
