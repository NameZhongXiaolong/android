 ...
 *                 this, // Context.
 *                 android.R.layout.two_line_list_item,  // Specify the row template to use (here, two columns bound
 *                 to the two retrieved cursor
 * rows).
 *                 mCursor,                                              // Pass in the cursor to bind to.
 *                 new String[] {People.NAME, People.COMPANY},           // Array of cursor columns to bind to.
 *                 new int[] {android.R.id.text1, android.R.id.text2});  // Parallel array of which template objects
...
public class ListActivity extends BaseActivity {
    ...
    public static final String KEY_ACTIVITY_TITLE = "ctivity_title";
    ...
    public void onContentChanged() {
        ...
        if (toolbar != null) {
            ...
            getSupportActionBar().setTitle(TextUtils.isEmpty(title)?"ZhongXiaolong":title);
        }
        if (mList == null) {
        ...
        mFinishedStart = true;
        setThemeColor(Color.parseColor("#CDA49E"));
    }
    ...
    public void setThemeColor(@ColorInt int color) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) getWindow().setStatusBarColor(color);
    }
     ...
     * @param fm
     */
    public void addFmToStack(Fragment fm) {
        String fmName = fm.getClass().getSimpleName();
        getSupportFragmentManager().beginTransaction()
                .setCustomAnimations(R.anim.fm_enter, R.anim.fm_enter, R.anim.fm_exit, R.anim.fm_exit)
                .addToBackStack(fmName).add(getFmContainerId(), fm, fmName).commitAllowingStateLoss();
    }

    @IdRes
    protected int getFmContainerId(){
        return R.id.fm_container;
    }

    protected Context mContent = this;
